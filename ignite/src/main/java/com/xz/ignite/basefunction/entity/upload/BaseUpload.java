...
public abstract class BaseUpload<K,V> {
        ...
        for (int i = 0; i < threadSize; i++) {
            ...
            completionService.submit(new Callable<String>() {
                ...
                public String call() throws Exception {
                    ...
                    long l1 = System.currentTimeMillis();
                    Map<K, BinaryObject> map = new HashMap<>();
                    for (long i = 1; i <= partSize; i++) {
                        V v = getBean();
                        map.put(getKey(v), IgniteUtil.toBinary(v));
                        if (i % pageSize == 0) {
                            submit(map, stmr);
                            map.clear();
    ...
    private void submit(Map<K, BinaryObject> map, IgniteDataStreamer<K, BinaryObject> stmr) {
        stmr.addData(map);
        stmr.flush();
    }
