package com.newland.boss.script.performance.transcation.pessimistic.repeatableread;
 ...
 */
public class PessimisticEPmanyPutScriptWork implements Callable<Long> {
    ...
    public PessimisticEPmanyPutScriptWork(EnterParam enterParam, Ignite ignite, IgniteCache<String,TranscationCache1> igniteCache1, IgniteCache<String,TranscationCache2> igniteCache2, Integer baseKey) {
    ...
    private Long working() {
        ...
        for (int i = 0; i < enterParam.getCount(); i++) {
            String randomKey1 = baseKey+i+"" ;
            String randomKey2 = baseKey+i+"" ;
        ...
        for (int i = 0; i < list1.size(); i++) {
            Transaction tx = igniteTransactions.txStart(TransactionConcurrency.PESSIMISTIC,
                    TransactionIsolation.REPEATABLE_READ, 5000, 2);
            try {
                String key1 = list1.get(i).field("id") ;
                String key2 = list2.get(i).field("id") ;
                ic1.invoke(key1,new PutEp1(),list1.get(i)) ;
                ic2.invoke(key2,new PutEp1(),list2.get(i)) ;
                tx.commit();
            } catch (Exception e) {
                e.printStackTrace();
            }finally {
