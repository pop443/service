 ...
 */
public class PartitionPutScript extends BaseScript<String,PartitionSmallCustObj> {
    private int count ;
    private int threadNum ;
    private ExecutorService executorService ;
    public PartitionPutScript(int count,int threadNum) {
    ...
    public void work() {
        ...
        for (int i = 0; i < threadNum; i++) {
            PartitionPutScriptWork partitionPutScriptWork = new PartitionPutScriptWork(eachSize,count,igniteCache) ;
            completionService.submit(partitionPutScriptWork);
        }
        long holeTime = 0 ;
        try {
            for (int i = 0; i < threadNum; i++) {
    ...
    public static void main(String[] args) throws Exception{
        ...
        System.out.println("数据量："+count+";线程："+threadNum);
        PartitionPutScript scirpt = new PartitionPutScript(count,threadNum) ;
        scirpt.start();
    }
